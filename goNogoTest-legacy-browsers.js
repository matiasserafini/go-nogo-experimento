...
async function experimentInit() {
  ...
  instr1 = new visual.TextStim({
    win: psychoJS.window,
    name: 'instr1',
    text: '\nA continuación, se presentarán rectángulos de color rojo y negro. Presione la barra espaciadora lo más rápido posible cuando aparezca el rectángulo rojo y absténgase de presionarla cuando aparezca el rectángulos negro.\n\nPresione la barra espaciadora para iniciar.',
    font: 'Arial',
    units: undefined, 
  ...
  return async function () {
    ...
    if (instr1Resp.status === PsychoJS.Status.STARTED) {
      ...
      if (_instr1Resp_allKeys.length > 0) {
        ...
        instr1Resp.duration = _instr1Resp_allKeys[_instr1Resp_allKeys.length - 1].duration;
...
function instructionsRoutineEnd(snapshot) {
  return async function () {
    ...
    if (currentLoop instanceof MultiStairHandler) {
      currentLoop.addResponse(instr1Resp.corr, level);
    }
    ...
    if (typeof instr1Resp.keys !== 'undefined') {  // we had a response
        ...
        psychoJS.experiment.addData('instr1Resp.duration', instr1Resp.duration);
        routineTimer.reset();
        }
